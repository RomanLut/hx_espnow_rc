...
Квадрокоптер управляетя приложением Xiro App. К сожалению, оно не работает на Android 10+. Ограничение дальности установлено в 100м, после чего дрон упирается в "виртуальную стену".
...
Но ограничение дальности увеличено до 200м.
...
# FPV видепоток с квадрокоптера

Приложение инициирует поток видео с порта 6000 кваркроптера на порт 7078 телефона посылкой пакета вида:

0xa0 0x09 0xe1 0x01 0x51 0x28 0xa6 0x1b 0xc5

в котором 0xa6 0x1b - номер порта 0x1ba6 = 7078, остальные данные не понятны. Для поддержания видеопотока нужно остылать такие пакеты каждые 3 секунды.
...
Канал 7: PTZ Работет только если в канал 6 посылается значение 0x00dc (? не подтверждено). Правое колёсико пульта Xiro Mini.
...
Для этого квадрокоптера было выпущено ограниченное количество пультов управления, с помощью которых можно летать до 500м с отображением видео и телеметрии на телефоне.
...
На экстендере пульта есть некий сервис, которые позволяет приложение определить, чот он присеодинен к сети пульта.
...
В идеальной реализации управления пультом нужно перехватывать пакты приложения от телефона и подменять пакеты управления.

В существующей реализации ESP32 присоединяется к дрону(192.168.1.1) интерфейсом STA, и открывает точку доступа для телефона на интерфейсе AP(192.168.4.1).
...
Чтобы как-то реализовать простотр видео, ESP32 пересылает все пакеты телеметрии (порт 7088) и видео(порт 7078) на телефон (192.168.4.2).
Приложение будет воспринимать эти пакеты, только если уже было соедение в дроном. То есть сначала нужно присоединиться д крону напрямую,
установить связь, а затем уже включить пульт и переключить телевон на сеть пульта. (Update: видео в приложении отображается, телеметрия нет).
Проверенно только с Xiro App.

Для инициирования передачи видеопотока на esp32 нужно посылать пакеты 0xa0 0x09 0xe1 ... каждые три секунды.
ESP справляется только с потоком 320x240.
...
Также есть следующая идея, возможно проще реаллизуемая:
Телефон подключается к AP 192.168.4.1 Gateway 192.168.4.1 и получает адрес 192.168.4.2. Приложение посылает пакеты дрону на адрес 192.168.1.1, котрые идут на default gateway 192.6.4.1(ESP32 AP interface).
Но эти пакеты невозможно принять на ESP32(192.168.4.1) на уровне сокетов. Однако, может быть можно включить sniffer callback и разбирать услышанные пкеты.
