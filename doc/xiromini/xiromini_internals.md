...
Квадрокоптер управляетя приложением Xiro App. К сожалению, оно не работает на Android 10+. 
...
На экстендере пульта есть некий сервис, которые позволяет приложению определить, что он присеодинен к сети пульта.
...
Чтобы как-то реализовать просмотр видео, ESP32 пересылает все пакеты телеметрии (порт 7088) и видео(порт 7078) на телефон (192.168.4.2).
