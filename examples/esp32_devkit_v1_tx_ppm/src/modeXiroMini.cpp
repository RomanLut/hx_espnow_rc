...
void ModeXiroMini::sendChannels( PPMDecoder* ppmDecoder )
{
    ...
    for ( int i = 0; i < 4; i++ )
    {
        r = ppmDecoder->getChannelValueInRange( i, 1100, 1900 );
        this->udpWriteWithChecksum( r >> 8, &checksum);
        this->udpWriteWithChecksum( r & 0xff, &checksum);
    }
    ...
    this->udpWriteSwitchWithChecksum(  ppmDecoder, 5-1, 0x3e8,  &checksum);
    ...
    //this->udpWriteWithChecksum( 0x03, &checksum);  
    //this->udpWriteWithChecksum( 0xE8, &checksum);
    this->udpWriteSwitchWithChecksum(  ppmDecoder, 6-1, 0x00dc, &checksum);
    ...
    this->udpWriteSwitchWithChecksum(  ppmDecoder, 7-1, 0x3e8,  &checksum);
    ...
    //this->udpWriteSwitchWithChecksum(  ppmDecoder, 8-1, 0x0003,  &checksum);
    ...
    //this->udpWriteWithChecksum( 0x03, &checksum);   
    //this->udpWriteWithChecksum( 0xE8, &checksum);
    this->udpWriteSwitchWithChecksum(  ppmDecoder, 9-1, 0x3e8, &checksum);
    ...
    this->udpWriteSwitchWithChecksum(  ppmDecoder, 10-1, 0x3e8, &checksum);
